...
exports = module.exports = {
  ...
  single: function(book) {
    try {
      build(book);
      if (book.sample && book.publish) {
        buildSample(book);
      }
    } catch (err) {
      console.log(book + '\'s build process fail.');
    }
  }
};
  ...
  function getSampleFile(book) {
    var outName = book.filename || book.book;
    var outFile;
    if (String(book.edit_version) !== "undefined") {
      outFile = path.join(book.outFolder, outName) + '-' + String(book.edit_version) + '-sample.epub';
    } else {
      outFile = path.join(book.outFolder, outName) + '-sample.epub';
    }
    return outFile;
  }
  ...
  function buildSample(book) {
    
    var outFile = getSampleFile(book);
    ...
    book['sample'].forEach(function(file){
      document += fs.readFileSync(path.join(book.folder, file), 'utf-8');
      document += '\n\n';
    });
    temp.track();
