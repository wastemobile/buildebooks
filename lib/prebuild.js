...
function prepare(config, startPath, filter) {
  ...
  _.each(projects, function(project){
    ...
    if (stat.isDirectory()) {
      prepare(config, projectfile, filter);
    } else if (projectfile.indexOf(filter)>=0) {
      getFullData(config, projectfile);  
    };
  });
}
...
function getFullData(config, projectfile) {
...
module.exports = function(config, startPath, filter) {
  try {
    prepare(config, startPath, filter);
    return db('books').where({build: true});
  } catch (err) {
    console.log('prebuild process fail.');
  }
};